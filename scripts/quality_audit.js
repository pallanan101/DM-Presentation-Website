...
async function loadContentReviewQueues() {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE_URL2}/queues/completed`, {
    ...
    queues.forEach(queue => {
      ...
      const assignedToMe = queue.total_completed_count || 0;
      ...
      row.innerHTML = `
                ...
                <td id="completed-count-${queue.queue_id}" class="px-6 py-4 whitespace-nowrap text-base font-semibold text-center text-green-600">${assignedToMe}</td>
...
async function loadPendingTasks(queueId, queueName, category) {
  ...
  container.innerHTML = `
    ...
    <h2 class="text-2xl font-bold mb-6">${queueName} – Pending Task Audit</h2>
      ...
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Moderated At</th>
  ...
  const resp = await fetch(`${API_BASE_URL2}/queues/${queueId}/completed-tasks`, {
    headers: { 'Authorization': `Bearer ${token}` }
  });
...
function addOrUpdateTaskRow(task, category) {
  ...
  const rowHTML = `
    <tr id="${rowId}">
      ...
      <td class="px-6 py-4">${new Date(task.moderated_at).toLocaleString()}</td>
      <td class="px-6 py-4">
        ${renderViolationDropdown(task.violation_id, task.content_id)}
      </td>
      <td class="px-6 py-4 text-center space-x-2">
        <button class="px-3 py-1 bg-blue-200 text-green-800 rounded" onclick="approve('${task.content_id}')">Review</button>
      </td>
    </tr>
  `;
